...
class ItemController:
    ...
    async def bulk_create(
        ...
        items_data = [schemas.Item(owner_id=owner_id, **item_data.model_dump()) for item_data in items_data]
        items = await models.Item.async_objects(async_session).bulk_create([item_data.model_dump() for item_data in items_data])
        for item in items:
